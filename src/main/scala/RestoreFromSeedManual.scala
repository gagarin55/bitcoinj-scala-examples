 ...
 * Usage:
 *    sbt "run-main com.wlangiewicz.RestoreFromSeedManual"
 *
 */
object RestoreFromSeedManual extends App {
  override def main(args: Array[String]): Unit = {
    ...
    // HD wallets allow you to restore your wallet simply from a root seed.
    // This seed can be represented using a short mnemonic sentence as described in BIP 39: https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki
    // Here we restore our wallet from a seed with no passphrase.
    // Also have a look at the BackupToMnemonicSeed example that shows how to backup a wallet by creating a mnemonic sentence.
    val seedCode: String = "yard impulse luxury drive today throw farm pepper survey wreck glass federal"
    val passphrase: String = ""
    val creationtime: Long = 1409478661L
    val seed: DeterministicSeed = new DeterministicSeed(seedCode, null, passphrase, creationtime)

    // The wallet class provides a easy fromSeed() function that loads a new wallet from a given seed.
    val wallet: Wallet = Wallet.fromSeed(params, seed)
    ...
    val chainFile: File = new File("restore-from-seed.spvchain")
    if (chainFile.exists) {
      chainFile.delete
    }

    // Setting up the BlochChain, the BlocksStore and connecting to the network.
    val chainStore: SPVBlockStore = new SPVBlockStore(params, chainFile)
